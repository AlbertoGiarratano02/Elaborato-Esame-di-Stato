...
?>
  <html>
   <head>
	 <meta charset="UTF-8">
     <title>Chiusura di un impianto</title>
	 <link rel="stylesheet" href="StyleChiusuraImpianto.css">
   </head>
    <body>
		<form method="POST" action="ChiusuraImpianto.php">
			<div class = "flex-container">
                <?php
                    ...
                    echo"</select><br><br>";
                ?>
				<button type="submit"> Chiudi Impianto </button><br><br>
			</div>
		</form>
    </body>
  </html>
...
<?php
	...
	if (strlen($codImpianto) != 0) {
		...
		$query = "UPDATE Impianto SET Impianto.Stato = 'false' WHERE Impianto.CodiceImpianto = '$codImpianto'";
		$result = $connection->query($query) or die("Errore query : $query");
		if ($result != 0){
			echo "<div class = 'flex-container'>";
			echo " L'impianto con il nome: $nome &egrave stato chiuso <br><br>";
			echo " <button><a href=\"Dipendenti.html\">Home Page </a></button><br>";
			echo "</div>"; 
		}
        else{
		   echo "<div class = 'flex-container'>";
		   echo " Impianto $nome non &egrave stato chiuso !<br><br>";
		   echo " <button><a href=\"ChiusuraImpianto.php\"> Riprova </a></button><br>";
		   echo "</div>";
        }
	}
	else
